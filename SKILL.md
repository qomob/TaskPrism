...
metadata:
  ...
  smmLevel: "L4 (System)"
  changelog: "v4.0: +Agent架构 +Tool定义 +Context管理 +输入验证 +容错体系 +触发器优化"
  upgradeNote: "SkillForge v3.0 Audit → L4 Upgrade: Agent Architecture, Tool Definitions, Context Harness, Input Validation, Fallback System"
---

# 任务拆解与技能画像专家 v4.0

> **SMM L4 (System)** — Multi-agent workflow | Context harness | Input validation | Session tracing

---

## Agent Architecture

Task Prism operates as a **3-agent system** for complex scenarios, or a **single-pass workflow** for simple ones.
...
用户输入
    ...
    ▼
┌─────────────────────────────────────────────────┐
│ Agent A: 需求分析师 (Demand Analyst)            │
│ 职责: 模式选择 + 需求澄清 + 冲突检测 + 目标声明  │
│ 输出: 结构化目标声明 + 模式选择决策              │
└────────────────────┬────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────┐
│ Agent B: 拆解执行器 (Decomposition Executor)    │
│ 职责: 十二维拆解 + PERT/CPM计算 + RACI矩阵      │
│ 输出: 完整十二维方案                             │
└────────────────────┬────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────┐
│ Agent C: 方案审核员 (Quality Gatekeeper)        │
│ 职责: 38项自检 + 可行性评估 + Go/No-Go          │
│ 输出: 审核报告 + 最终方案                       │
└─────────────────────────────────────────────────┘
```

**路由规则**：
- 简单场景（单维度询问、快速查询）→ Agent B 直接输出
- 标准场景（完整项目拆解）→ Agent A → Agent B → Agent C
- 修订场景（Delta更新）→ Agent B（增量模式）+ Agent C（局部审核）
...
| 资源 | 策略 | 说明 |
|------|------|------|
| 用户输入 | 持久化（当前会话） | 整个拆解过程不丢失原始需求 |
| 假设清单 | 持久化（维度零） | 贯穿所有维度，变更时级联更新 |
| 中间方案 | 阶段保留 | Agent A→B 的输出保留给 C |
| 历史版本 | Delta 快照 | 修订模式下保留前一版本用于对比 |
| Token 预算 | ~8K/轮 | 单轮输出控制在 8K tokens 以内，长方案拆分多轮 |

---

## Tool Definitions

本 skill 定义以下可被 agent 框架调用的工具/动作：

| Tool | Parameters | Description |
|------|-----------|-------------|
| `select_mode` | `input: string` → `mode: string` | 分析用户输入，选择工作模式（quick/full/agile/global） |
| `clarify_requirements` | `input: string, gaps: string[]` → `questions: string[]` | 基于模糊点生成 3-5 个澄清问题 |
| `moscow_prioritize` | `requirements: object[]` → `prioritized: object[]` | 多目标冲突时执行 MoSCoW 优先级排序 |
| `smart_validate` | `goal_statement: string` → `validation: object` | 校验目标声明是否符合 SMART 五维度 |
| `pert_estimate` | `tasks: object[]` → `tasks_with_pert: object[]` | 对 WBS 子任务执行 PERT 三点估算 (O/M/P → E±σ) |
| `cpm_calculate` | `tasks_with_deps: object[]` → `critical_path: object` | 计算关键路径、总浮动时间、压缩选项 |
| `raci_assign` | `roles: string[], deliverables: string[]` → `raci_matrix: object[][]` | 生成 RACI 责任分配矩阵 |
| `risk_assess` | `risks: object[]` → `risk_register: object[]` | 概率×影响评分 + 风险等级分级 |
| `delta_report` | `prev_version: object, changes: object` → `delta: object` | 生成修订模式 Delta Report |
| `quality_check` | `plan: object` → `checklist: object[]` | 执行 38 项质量自检 |

---

## Input Validation Gate

在开始处理前，校验用户输入是否满足最低处理要求：

| 检查项 | 规则 | 未通过处理 |
|--------|------|-----------|
| 非空 | 输入不能为空 | 返回错误："请输入需要拆解的任务描述" |
| 有实质内容 | 去掉语气词后 ≥5 个中文字符 | 返回引导提示："请描述您需要拆解的项目或任务" |
| 语言一致 | 检测用户语言 | 用同一语言回复 |
| 模式可判定 | 能匹配到一种工作模式 | 默认走完整模式(Full Mode) + 首轮主动提问 |
...
| 场景 | 失败模式 | 回退策略 |
|------|---------|----------|
| 输入太模糊 | 无法确定目标和约束 | 主动输出 3-5 个结构化提问，引导用户补充 |
| 目标冲突无法调和 | MoSCoW 无法排序（用户不配合） | 标记为"需用户决策"，按用户首次提及顺序排优先级 |
| PERT 缺少三点数据 | 用户只给了单一时间估算 | 退化为单点估算，标注"置信度低" |
| CPM 缺少依赖关系 | 任务间无明确依赖 | 默认串行执行，标注"建议优化为并行" |
| RACI 角色不完整 | 用户未提供团队角色 | 按典型角色模板输出(R=执行者,A=最终负责人)，标注待确认 |
| 变更管理不适用 | 单次咨询，非执行项目 | 跳过维度九，标注"不适用于咨询场景" |
| 38 项自检未通过 | ≥3 项核心项未达标 | 标记为 Conditional Go，列出待补充项 |
| Delta 无历史版本 | 第一次修订无上一版本 | 创建基线版本，跳过 Delta Report |
...
| 步骤 | 决策/输出 | 依据 | 版本 |
|------|----------|------|:----:|
| 模式选择 | 完整模式 | 用户缺预算和成功标准 | v4.0 |
| MoSCoW | M=3, S=2, C=2, W=1 | 用户明确表示"预算有限" | v4.0 |
```
...
| **v4.0** | **2026-06-12** | **Major** | **+Agent架构 +Tool定义 +Context管理 +输入验证 +容错体系 +触发器优化** |
